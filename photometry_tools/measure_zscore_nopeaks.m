...
dff_clean_color = [132/255, 221/255, 199/255];
...
xlabel('Time');
ylabel('Amplitude');
legend('470 nm', '405 nm');


subplot(3,1,2)
hold on
plot(t, dff, 'color', dff_color);
plot(t, dff_clean, 'color', dff_clean_color);
xlabel('Time');
ylabel('Amplitude');
legend('dff', 'dff w/o peaks');

subplot(3,1,3)
hold on
plot(t, z_score, 'color', dff_color);
for i=1:n_pks
    x = locs(i);
    plot(t(x), pks(i), 'v', 'MarkerFaceColor', dff_color, 'MarkerEdgeColor', 'none');
end

 mean_bl = nanmean(dff_clean);
 std_bl = nanstd(dff_clean);
 z_score_clean = (dff - mean_bl) / std_bl;

plot(t, z_score_clean, 'color', dff_clean_color);

xlabel('Time');
ylabel('Amplitude');

legend('z-score', 'peaks>2.58', 'clean z-score');
