function experiment = eventBasedAnalysis(experiment)
...
if isfield(experiment.p,'extract_bites_from_audio')
    if(experiment.p.extract_bites_from_audio==1)
...
if isfield(experiment.p,'extract_events_from_kdenlive')
        if(experiment.p.extract_events_from_kdenlive==1)
            filename = [experiment.p.dataRoot filesep experiment.p.dataFileTag '.kdenlive']
            import_kdenlive_xml(filename)
            default_analysis = 0;
        end
     
end

if isfield(experiment.p,'extract_events_from_eventstxtfile')
        if(experiment.p.extract_events_from_eventstxtfile==1)
            filename = [experiment.p.dataRoot filesep experiment.p.dataFileTag '_events.txt']     
            idx_synchro = load(filename)
            default_analysis = 0;
        end
end
